...
signal damage_feedback(target: Node2D, amount: float, position: Vector2, is_player: bool)
signal player_died()
...
signal safehouse_discovered(house_id: int)
...
signal throwable_thrown(throwable_type: int, position: Vector2, direction: Vector2, source: Node2D, charge_ratio: float)
